...
class ProjectileActionComposer
{
    public static void ComposeAction(ActionProposal proposal, out ActionResult result, out Timeline<ActionEvent> timeline)
    {
        ...
        AnimationPlaceholder shootArrowAnimation = AnimationFactory.CheckoutAnimation(AnimationId.BowDraw);
        timelineEvents.Add(new AnimationEvent(owner, shootArrowAnimation, 0, proposal.ActionTarget.FocalTarget.GetTargetTransform()));
        timelineEvents.Add(new AudioEvent(ownerController.Actor.AudioSource, SFXId.ArrowDraw, 0));
        timelineEvents.Add(new AudioEvent(ownerController.Actor.AudioSource, SFXId.ArrowRelease, shootArrowAnimation.SyncedFrame));
        ...
        ProjectileSpawnParams spawnParams = EncounterModule.ProjectileDirector.GenerateSpawnParams(projectileSpawnPoint, projectileEndPoint, ProjectilePathType.Arc, ProjectileId.Arrow);
        timelineEvents.Add(new ProjectileSpawnEvent(spawnParams, shootArrowAnimation.SyncedFrame));
        ...
        if (targets.Count > 0)
        {
            ...
            SyncPoint.Syncronize(shootArrowAnimation, AllignmentPosition.Interaction, animation, AllignmentPosition.Interaction, AnimationConstants.FRAMES_IN_DURATION(spawnParams.FlightDuration));
