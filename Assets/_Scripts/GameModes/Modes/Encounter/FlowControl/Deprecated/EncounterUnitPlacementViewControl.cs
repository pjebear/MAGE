...
namespace MAGE.GameModes.FlowControl
{
    ...
    , IInputHandler
    {
        ...
        public void Start()
        {
            //int numPlacedAllies = GameModel.Encounter.Teams[TeamSide.AllyHuman].Count;
            //int maxAllies = GameModel.Encounter.EncounterContext.MaxAllyUnits;
            //if (numPlacedAllies < maxAllies)
            //{
            //    foreach (int characterId in WorldService.Get().GetCharactersInParty())
            //    {
            //        Character character = MAGE.GameSystems.CharacterService.Get().GetCharacter(characterId);
            //        character.TeamSide = TeamSide.AllyHuman;

            //        if (GameModel.Encounter.EncounterContext.CharacterPositions.ContainsKey(character.Id))
            //        {
            //            TileIdx atTile = GameModel.Encounter.EncounterContext.CharacterPositions[character.Id];
            //            CharacterPosition characterPosition = new CharacterPosition(atTile, Orientation.Forward);
            //            EncounterFlowControl_Deprecated.CharacterDirector.AddCharacter(character, characterPosition);
            ...
            //{
            //    foreach (TileIdx tileIdx in GameModel.Encounter.AllySpawnPoints)
            //    {
            //        mPlacementTiles.Add(EncounterFlowControl_Deprecated.MapControl[tileIdx]);
            //    }

            //    mTileSelectionStack.UpdateLayer(mAvailableTileLayer, mPlacementTiles);
            //    mTileSelectionStack.DisplayTiles();

            //    EncounterFlowControl_Deprecated.CameraDirector.FocusTarget(mPlacementTiles[0].transform);
        ...
        private void UpdateSelectableTiles()
        {
            ...
            //    {
            //        availableTiles.Add(EncounterFlowControl_Deprecated.MapControl[tileIdx]);
            //    }
            //}
        ...
        private void PlaceSelectedCharacterAtTile(TileControl tile)
        {
            ...
            //        CharacterActorController controller = EncounterFlowControl_Deprecated.CharacterDirector.CharacterActorLookup[GameModel.Encounter.Characters[selectedCharacterId]];

            //        EncounterFlowControl_Deprecated.MapControl.UpdateCharacterPosition(controller, new CharacterPosition(tile.Idx, Orientation.Forward));
            ...
            //    EncounterFlowControl_Deprecated.CharacterDirector.AddCharacter(mCharactersToPlace[mCharacterIdx], new CharacterPosition(tile.Idx, Orientation.Forward));
            //}
        }
